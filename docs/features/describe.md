...
- 在 History 的 Describe 分组中选择 `Generate ALL` 时，会逐条把描述 prompt 作为新的生成任务输入，并沿用当前 Playground 的模型与生成参数，而不是回填整组历史参数。
...
- Describe 会产出可复用 prompt，但它属于 `describe_image` 流程，不属于 prompt optimization；底层执行入口是 `/api/ai/describe` -> `service:describe`，不是 `/api/ai/text`。
...
- 2026-04-08：明确 Describe 历史分组批量动作文案为 `Generate ALL`，并说明该动作会按当前 Playground 配置逐条发起新生成。
