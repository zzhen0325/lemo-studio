...
- SSR 边界：Playground Dock 通过 client-only 动态加载 `GalleryView`，避免 `masonic` / `react-virtualized-auto-sizer` 在预渲染阶段触发 `ResizeObserver`；本地 fixture 直接注入静态数据，不依赖 `/api/history`，并在本地排查闪动时改为 client-only 挂载，避免 dev SSR fallback 把错误页与真实图墙叠在一起。
...
- 卡片：`GalleryImageCard` 接收统一的 view model 和 action handlers，下载统一使用解析后的 download URL；虚拟化图墙优先使用 `previewUrl`（强缓存缩略图）做轻量渲染，失败时再回退到原图或参考缩略图，并避免在 `masonic` 路径叠加 `next/image` blur placeholder 与长过渡动画。
