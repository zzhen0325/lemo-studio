...
- 本地布局 fixture：`/studio/gallery/local` 直接读取仓库根目录 `test gallery/` 图片，并默认通过同一套 `GalleryScene` 渲染链做人工布局验收；在本地闪动排查阶段可临时切换到非虚拟化静态 columns 图墙，用来隔离 `masonic` 布局/重挂载带来的视觉噪音。
- SSR 边界：Playground Dock 通过 client-only 动态加载 `GalleryView`，避免 `masonic` / `react-virtualized-auto-sizer` 在预渲染阶段触发 `ResizeObserver`；首次打开后 Dock 会保留 Gallery 实例，仅在切走时改为隐藏态 keep-alive，并把外层动态加载 fallback 与内部图墙统一为同款骨架；本地 fixture 直接注入静态数据，不依赖 `/api/history`，并在本地排查闪动时改为 client-only 挂载，避免 dev SSR fallback 把错误页与真实图墙叠在一起。
...
- `GET /api/history?id=...` / `GET /api/history?outputUrl=...`：图片详情弹窗补拉完整 generation detail，返回单条 `item`，供 `Use All` / `Rerun` 等高风险复用动作使用。
...
- Gallery 的最终展示列表 = `optimistic overlay + SWR gallery feed`，按记录 id 去重；当服务端记录出现后，展示层会优先使用服务端版本。
- Zustand 只承担 Playground 本地 UI / editor state；Gallery feed 不再进入 store，也不再参与持久化。
- Dock 层会记住 Gallery 是否至少打开过一次；打开后切走仅隐藏不卸载，因此本地 `sortBy`、filters（包括“所有/精选”）、inner tab 与 masonry 布局实例都会保留。
- 视图层以 `GalleryItemViewModel` 为唯一输入，统一处理展示 URL、下载 URL、thumbnail、Prompt 分类和图墙收录规则。
- Gallery feed 返回 lightweight/minimal history 记录时，需要显式标记为可补拉 detail；图片详情弹窗中的高风险复用动作（如 `Use All` / `Rerun`）必须等 detail hydration 完成后再执行。
...
- 2026-04-09：Gallery feed 展示层补充 optimistic history overlay 合并，减少新生成记录等待服务端 revalidate 才出现的延迟；首次请求失败后的重试不再回退到初始 skeleton。
...
- 2026-04-09：`/studio/gallery/local` 调试入口改为 client-only 挂载，并临时跳过全页 `PageTransition` 与卡片 blur/fade 占位动画，便于隔离图片闪动与整页位移来源。
- 2026-04-09：`/api/dev/test-gallery` 本地 fixture 图片改为强缓存，避免 Masonry 滚动重挂载时对同一张本地图片重复发起请求，干扰闪动排查。
- 2026-04-09：`/studio/gallery/local` 排查闪动时临时绕开 `masonic`，改为非虚拟化静态 columns 图墙，便于确认滚动后高频闪动是否来自 Masonry 定位与重挂载链。
- 2026-04-09：正式 Gallery 的 `masonic` 图墙改为 client-only 挂载，并让虚拟化卡片优先使用 `/api/storage/image?w=...` 生成的强缓存缩略图与原生 `<img>` 轻量渲染，降低滚动重挂载时的大图解码和动画闪动。
- 2026-04-09：正式 Gallery 的虚拟化卡片进一步移除 hover scale、参考图缩放与工具栏 `backdrop-blur` 等高合成成本效果，减少 `masonic` 滚动中的整屏光栅化失败风险。
