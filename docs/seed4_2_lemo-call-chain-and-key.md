# seed4_v2_0226lemo 调用方法与 Key 整理

## 1. 快速结论
- 真正发起 `seed4_v2_0226lemo` 生图请求的服务端方法是 `BytedanceAfrProvider.generateImage`。
- 该模型请求不依赖 `provider.apiKey` 字段，使用的是签名参数：
  - `GATEWAY_BASE_URL`
  - `BYTEDANCE_AID`
  - `BYTEDANCE_APP_KEY`
  - `BYTEDANCE_APP_SECRET`
- `data/api-config/providers.json` 中 `provider-bytedance.apiKey` 当前为空，且当前代码路径不会读取它用于 `seed4_v2_0226lemo`。
...
## 3. seed4_v2_0226lemo 的直接命中点

### 3.1 运行时相关（会影响请求行为）
- `lib/ai/registry.ts:76`  
  注册 `id: 'seed4_v2_0226lemo'`，providerType 为 `bytedance-afr`。
- `lib/ai/providers.ts:556`  
  `isSeed42 = this.config.modelId === "seed4_v2_0226lemo"`。
- `lib/ai/providers.ts:563`  
  `seed4_v2_0226lemo` 分支设置：
...
结论：`seed4_v2_0226lemo` 当前鉴权逻辑是“签名参数模式”，不是“Bearer API Key 模式”。
