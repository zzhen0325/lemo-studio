# LEMO Studio（Crate / Lemon8 AI Studio）产品理念与技术方案
...
- 名词映射：本文中提到的 Crate，指向同一套 LEMO Studio 创作工作台能力
...
### 9.1 Playground 主界面截图

![Playground 主界面截图](./assets/product-philosophy/playground-home-overview.png)

### 9.2 Playground 到 Moodboards 的交互示例

![Playground 到 Moodboards 的交互 GIF](./assets/product-philosophy/playground-home-to-moodboards.gif)

## 十、Crate 目录对齐补充（参考截图）

> 以下内容用于对齐“什么是 Crate / 如何使用 Crate 进行创作”的培训目录口径，和上文产品与技术方案保持一致。

### 10.1 什么是 Crate？

Crate 是一套面向业务创作的 AI 生产工作台。它把“输入任务、生成结果、组织资产、复用流程”放在同一平台里，让团队从单次出图转向持续产出。

#### 10.1.1 它在业务中能做到哪些事？

1. 快速做多版创意探索：同一主题下并行验证风格、构图和模型策略。
2. 把一次性结果变成可复用资产：沉淀到 History、Dataset、Preset、项目画布。
3. 支持从灵感到交付的闭环：生成、筛选、编辑、组合、导出、分享。
4. 降低高级工作流使用门槛：通过 Mapping 把复杂节点能力抽象成可配置面板。
5. 提升团队协同效率：围绕项目和文件组织结果，减少口头传参与重复试错。

#### 10.1.2 平台基础能力

1. 多模型与多链路路由能力：同源 API 统一入口，Provider 差异在服务层消化。
2. 创作上下文能力：Prompt、参考图、Preset、历史结果可组合注入。
3. 项目化编排能力：Infinite Canvas 承接节点化组织与多轮迭代。
4. 资产沉淀能力：生成结果可进入 Dataset、History、Gallery，支持回溯复用。
5. 可扩展工作流能力：Workflow Mapping 支持将底层链路映射为业务可操作配置。

### 10.2 Crate 工具基本概念

1. 文件（File）：承载一次任务或一组结果的组织单元，强调可追溯和可复用。
2. 项目（Project）：围绕目标主题聚合多个文件、模块与版本输出。
3. 画布（Canvas）：项目内进行可视化编排与对比决策的空间。
4. 模块（Module）：画布上的可操作单元，如文字、图片、视频、3D 等。
5. 模板工作流（Template Workflow）：把可复用配置固化成标准生产路径。
6. 智能体（Agent）：在明确边界下辅助执行高频操作（例如整理、推荐、填充）。

### 10.3 如何使用 Crate 进行创作

#### 10.3.1 文件和文件管理

1. 先按业务目标创建文件或项目（例如“春季活动主 KV”）。
2. 每轮生成都保留 Prompt、参数、模型和来源，确保可回放。
3. 使用统一命名和标签标识阶段（草稿、待选、终稿、已发布）。

#### 10.3.2 项目画布基本操作

1. 把关键结果拖入画布按主题分区摆放，形成方案对照。
2. 用连线或分组表达“从哪个版本演进而来”，保留决策路径。
3. 对确定方向的节点沉淀为模板或数据集，减少下次冷启动成本。

#### 10.3.3 模块和模块操作

1. 模块基本操作：新增、复制、替换、锁定、分组、批量调整参数。
2. 文字模块：用于补充标题、卖点、约束词和版本说明，便于协作评审。
3. 图片模块：支持参考图、结果图、草图等多来源素材统一编排。
4. 多图层编辑（Beta）：面向前景/背景分层调整、局部替换和层级控制。
5. 视频模块：用于承接动态素材、分镜帧和动效预览链路。
6. 3D 模块（Beta）：用于空间构图、角度尝试和跨媒介视觉探索。

### 10.4 模板工作流（New）

1. 把高成功率参数组合固化为模板，降低新任务起步成本。
2. 模板应包含适用场景、输入字段、输出标准和禁用边界。
3. 优先沉淀“高频、稳定、可解释”的工作流，避免黑盒不可复现。

### 10.5 群组功能（New）

1. 用群组组织同主题多版本（如同一 campaign 的不同风格线）。
2. 群组内保持统一评审维度，便于快速横向比较。
3. 群组结果可批量进入 Dataset 或导出给下游使用。

### 10.6 贴纸功能（New）

1. 贴纸用于快速添加视觉元素、标记信息或运营说明。
2. 贴纸应支持样式复用和位置规范，避免画布信息噪音。
3. 贴纸内容可作为后续 Prompt/描述生成的辅助上下文。

### 10.7 智能体（Agent）Beta

1. 智能体负责执行规则明确、重复性高的辅助任务。
2. 人工仍保留目标定义与结果决策，避免“自动化替代判断”。
3. 推荐先从低风险场景试点：素材整理、候选排序、模板推荐。

### 10.8 分享与查看

1. 分享对象不只是一张图，还应包含关键上下文（Prompt、参数、版本来源）。
2. 查看态要支持“快速理解 + 可追溯回放”，便于评审与复用。
3. 对外分享默认走脱敏策略，保障项目与素材安全边界。
