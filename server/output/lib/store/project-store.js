...
class ProjectStore {
    ...
    constructor() {
        ...
        (0, mobx_1.reaction)(() => user_store_1.userStore.currentUser?.id, () => {
            this.loadProjects();
    ...
    async loadProjects() {
        const currentUser = user_store_1.userStore.currentUser;
        if (!currentUser)
            return;
        try {
            const userId = currentUser.id;
            const res = await fetch(`${(0, api_base_1.getApiBase)()}/projects?userId=${userId}`);
            if (res.ok) {
    ...
    async saveProjects() {
        ...
        try {
            ...
            await fetch(`${(0, api_base_1.getApiBase)()}/projects?userId=${userId}`, {
                method: 'POST',
    ...
    addHistoryToCurrentProject(historyItem) {
    ...
    setProjectHistory(projectId, history) {
    ...
    async addGenerationsToProject(projectId, generations) {
        const project = this.projects.find(p => p.id === projectId);
        if (project) {
    ...
    async createProjectWithHistory(name, generations) {
