...
// 兼容旧格式的辅助函数
function migrateOldSettings(data) {
    // 如果已经是新格式
    if (data.services) {
        return data;
    }
    // 从旧格式迁移
    const oldBindings = data.serviceBindings;
    return {
        services: {
            imageGeneration: {
                binding: { providerId: 'provider-google', modelId: 'gemini-3-pro-image-preview' }
...
exports.useAPIConfigStore = (0, zustand_1.create)((set, get) => ({
    providers: [],
    ...
    fetchConfig: async () => {
        ...
        try {
            const response = await fetch(`${(0, api_base_1.getApiBase)()}/api-config`);
            if (!response.ok)
    ...
    removeProvider: async (id) => {
        set({ isLoading: true, error: null });
        try {
            const response = await fetch(`${(0, api_base_1.getApiBase)()}/api-config?id=${id}`, {
                method: 'DELETE'
            });
            if (!response.ok)
