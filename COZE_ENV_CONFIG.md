...
### ComfyUI 配置
⚠️ 注意：`10.75.163.10` 是局域网地址，云端部署无法访问！
需要使用 ngrok/frp 等工具暴露到公网后替换地址。

```
COMFYUI_API_URL=https://10.75.163.10:1000
NEXT_PUBLIC_COMFYUI_URL=https://10.75.163.10:1000
COMFYUI_SECURE=true
```
...
### Coze (使用 LEMO_ 前缀)
```
LEMO_COZE_API_TOKEN=你的TOKEN
LEMO_COZE_PROMPT_API_TOKEN=你的TOKEN
LEMO_COZE_SEED_API_TOKEN=你的TOKEN
LEMO_COZE_PROMPT_RUN_URL=https://xxx.coze.site/run
LEMO_COZE_SEED_RUN_URL=https://xxx.coze.site/run
```
...
LEMO_COZE_API_TOKEN=填写你的值
LEMO_COZE_PROMPT_API_TOKEN=填写你的值
LEMO_COZE_SEED_API_TOKEN=填写你的值
LEMO_COZE_PROMPT_RUN_URL=https://xxx.coze.site/run
LEMO_COZE_SEED_RUN_URL=https://xxx.coze.site/run
...
1. **ComfyUI 地址**：云端部署无法访问局域网地址 `10.75.163.10`
