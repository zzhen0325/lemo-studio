...
export const PresetGridOverlay: React.FC<PresetGridOverlayProps> = ({ onOpenManager, onSelectPreset, externalPresets }) => {
    ...
    return (
        <motion.div
            ...
            exit={{ opacity: 0, y: 20 }}
            className="w-full max-w-4xl mx-auto mt-4 flex flex-col gap-2"
        >
            {/* Header: Tabs & Manager Entry */}
            <div className="flex items-center justify-between ">
                <div className="flex items-center gap-1 bg-black/20 border border-white/10 rounded-2xl p-1 backdrop-blur-md">
                    {CATEGORIES.map(cat => (
                        <button
                            key={cat}
            ...
            <div className="bg-transparent p-1">
                {filteredPresets.length === 0 ? (
                    ...
                    <div className="flex gap-2 overflow-x-auto rounded-xl snap-x scrollbar-hide">
                       
                        {filteredPresets.map(preset => (
                            <button
                                key={preset.id}
                                className="group relative flex flex-col items-start overflow-hidden bg-black/20 hover:bg-black/40 transition-colors rounded-xl p-2 min-w-[160px] w-[160px] snap-start border border-white/20"
                                onClick={() => onSelectPreset(preset)}
                            >
                                
                                <div className="relative w-full aspect-[4/5] rounded-md bg-white/20 overflow-hidden">
                                    {preset.cover ? (
